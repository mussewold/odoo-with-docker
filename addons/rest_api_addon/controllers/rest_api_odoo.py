...
class RestApi(http.Controller):
    ...
    def generate_response(self, method, model, rec_id):
        ...
        _logger.info("GenerateResponse method: %s", method)
        _logger.info("GenerateResponse model: %s", model)
        _logger.info("GenerateResponse rec_id: %s", rec_id)

        raw_request_data = json.loads(request.httprequest.data)
        _logger.info("Raw Request Data: %s", raw_request_data)
        ...
        if method != 'DELETE':  
            ...
            data = {}
        _logger.info("GenerateResponse Data: %s", type(data))
        _logger.info("GenerateResponse Data: %s", data['params']['fields'])
        fields = []
        if data:
            for field in data['params']['fields']:
                fields.append(field)
            ...
            if method == 'GET':
                ...
                if not option.is_get:
                    ...
                    if rec_id != 0:
                        ...
                        # Manually convert datetime and date fields to string format
        ...
        if method == 'POST':
            if not option.is_post:
                ...
                try:
                    ...
                    _logger.info("POST MODEL: %s", str(model_name))
                    _logger.info("POST VALUES: %s", data['params']['values'])
                    new_resource = request.env[str(model_name)].create(data['params']['values'])
                    _logger.info("POST LOG: %s",new_resource )
                    partner_records = request.env[
                    ...
                    # Convert datetime and date fields to string format
                    for record in partner_records:
                        for key, value in record.items():
                            if isinstance(value, (datetime, date)):
                                record[key] = value.isoformat()
